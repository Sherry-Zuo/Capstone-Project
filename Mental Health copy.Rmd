...
chart1<-maindata%>%
   ...
   ggplot(aes(x=factor(interfere, levels=c("Never","Rarely","Sometimes","Often")), 
      ...
      labs(x="Work Interfere", y="Probability", 
           title="Probability of Mental Health Interfere based on Gender")+
      theme(panel.background = element_rect(fill="transparent"),
        axis.line = element_line(colour = "black"))
```
```{r}
chart2<-maindata%>%
   ...
   ggplot(aes(x=factor(company_size, levels=c("1-5","6-25","26-100","100-500","500-1000","More than 1000")),
      ...
      labs(x="Company Size", y="Probability", 
           title="Probability of Mental Health Company Size base on Tech")+
      theme(panel.background = element_rect(fill="transparent"),
        axis.line = element_line(colour = "black"))
```
```{r}
chart3<-maindata%>%
   ...
   ggplot(aes(x=factor(medical_leave_easy, levels=c("Don't know","Very difficult","Somewhat difficult","Somewhat easy","Very easy")), y=proportion, fill=remote))+
      ...
      labs(x="Leave Easy", y="Probability", 
           title="Probability of Mental Health Leave Easy based on remote")+
      theme(panel.background = element_rect(fill="transparent"),
        axis.line = element_line(colour = "black"))
```
```{r}
...
```
```{r}
newdata <- merge(x=newdata, y=happiness, by="country")
```
```{r}
head(newdata)
```
...
```{r}
newdata <- merge(x=newdata, y=suicide, by="country")
```
```{r}
newdata
```
...
```{r}
summary(lm(mh_treatment~h_score+suicide_rate, data=newdata))
```
```{r}
dim(maindata)
```
...
mh_reg2 = dummy_cols(mh_reg, c("gender", "country", "family_history",
                     "tech_company", "mh_benefits", "mh_resources"))
mh_reg2 = mh_reg2[,c(2,5,9,10,12,19,23,26)]
...
```{r}
mod4 = lm(mh_treatment ~ . , data = mh_reg)
summary(mod4)
```

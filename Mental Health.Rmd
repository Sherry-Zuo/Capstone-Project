...
```
```{r}
maindata$region <- ifelse(maindata$state %in% c("CT", "ME", "MA", "NH", "RI", "VT", "NJ", "NY", "PA"), "NorthEast",
                           ifelse(maindata$state %in% c("IL", "IN", "MI", "OH", "WI", "IA", "KS", "MN", "MO", "NE", "SD"),"MidWest",
                                  ifelse(maindata$state %in% c("DE", "FL", "GA", "MD", "NC", "SC", "VA","DC", "WV", "AL", "KY", "MS", "TN", "TX", "OK", "LA", "AR"), "South",
                                         ifelse(maindata$state %in% c("AZ", "CO", "ID", "MT", "NV", "NM", "UT", "WY","AK", "CA", "HI", "OR", "WA"), "West", NA))))
```
```{r}
w<-table(maindata$region)
w
```
```{r}
str(w)
regions_count<-as.vector(w)
regions_percentage<-names(w)
regions_count
regions_percentage
```
```{r}
pct23 <- round(regions_count/sum(regions_count)*100)
regions_percentage <- paste(regions_percentage, pct22) # add percents to labels
regions_percentage <- paste(regions_percentage,"%",sep="") # ad % to labels
pie(regions_count,labels = regions_percentage, col=rainbow(length(regions_percentage)),
   main="Pie Chart of Regions")
```
```{r}
#remove state variable since we want to use regions
maindata<-maindata[-3]
```
...
dummydata<-dummy_cols(dummydata, select_columns = c("anonymity_protected", "awareness_mh_benefits","gender","medical_leave_easy", "mh_benefits", "mh_discuss", "mh_resources", "mh_serious_ph","region"))
```
```{r}
#after adding dummies, remove original categorical variables
#leave numeric variables
dummydata <- dummydata[-c(2,11:17,23,25)]
```
```{r}
dim(dummydata)
names(dummydata)
...
```{r}
set.seed(666)
# assign 118 random rows to the test set(around 30% of our dataset)
test_index <- sample(nrow(dummydata), 176) 
# now split
...
<!-- set.seed(666) -->
<!-- # assign 118 random rows to the test set(around 30% of our dataset) -->
<!-- test_index2 <- sample(nrow(maindata), 176) -->
<!-- # now split -->
<!-- mh.test <- maindata[test_index2,] -->
<!-- mh.train <- maindata[-test_index2,] -->
<!-- ``` -->

<!-- ```{r} -->
<!-- mh_test <- mh.test %>% filter(sample(c(0,1),nrow(mh.test),replace=TRUE,prob=c(0.95,0.05))==1) -->
<!-- mh_train <- mh.train %>% filter(sample(c(0,1),nrow(mh.train),replace=TRUE,prob=c(0.95,0.05))==1) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- y_train2 <- maindata$mh_treatment[-test_index2] -->
<!-- y_test2 <- maindata$mh_treatment[test_index2] -->
<!-- ``` -->

<!-- ```{r} -->
<!-- f1 <- as.formula(mh_treatment ~ age+gender+mh_benefits+tech_company+mh_discuss+company_size) -->
<!-- ``` -->

<!-- ```{r} -->
<!-- fit.tree <- rpart(f1, mh_train, -->
...
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat.train.tree <- predict(fit.tree, mh_train) -->
<!-- mse.train.tree <- mean((mh_train$mh_treatment - yhat.train.tree)^2) -->
<!-- mse.train.tree -->
<!-- ``` -->
<!-- ```{r} -->
...
<!-- f2 <- as.formula(mh_treatment ~ .) -->
<!-- ``` -->
<!-- ```{r} -->
<!-- set.seed(666) -->
<!-- x0 <- mh_train[1,] -->
<!-- mh_train_1 <- mh_train[-1,] -->
<!-- yhat_small_tree <- c() -->
<!-- for (i in seq(100)) { -->
<!--   fit_tree <- rpart(f2, -->
<!--     mh_train_1 %>% sample_frac(size = .1), -->
...
<!-- errors <- data.frame( -->
<!--   "error"= (x0$mh_treatment - c(yhat_small_tree, yhat_big_tree)), -->
...
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat.train.tree_2 <- predict(fit.tree_2, mh_train) -->
<!-- mse.train.tree_2 <- mean((mh_train$mh_treatment - yhat.train.tree_2)^2) -->
<!-- mse.train.tree_2 -->
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat.test.tree_2 <- predict(fit.tree_2, dd_test) -->
<!-- mse.test.tree_2 <- mean((mh_test$mh_treatment - yhat.test.tree_2)^2) -->
<!-- mse.test.tree_2 -->
<!-- ``` -->
...
<!-- x1_train <- model.matrix(f1, mh.train2)[, -1] -->
<!-- x1_test <- model.matrix(f1, mh.test2)[, -1] -->
<!-- ``` -->
...
<!-- ```{r} -->
<!-- fit_rf2 <- randomForest(f2, mh.train2, -->
<!--                   ntree=100, -->
<!--                   do.trace=F) -->
<!-- ``` -->
<!-- ```{r} -->
...
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat_rf2 <- predict(fit_rf2, mh.train2) -->
<!-- mse_rf_big <- mean((yhat_rf2 - y_train2) ^ 2) -->
<!-- print(mse_rf_big) -->
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat_rf2_test <- predict(fit_rf2, mh.test2) -->
...
<!-- ##### Boosted trees -->
<!-- ```{r} -->
<!-- fit_btree_small <- gbm(f1, data = mh.train, -->
...
<!-- ```{r} -->
<!-- yhat_btree_small <- predict(fit_btree_small, mh.train, n.trees = 100) -->
<!-- mse_btree_small <- mean((yhat_btree_small - y_train2) ^ 2) -->
<!-- print(mse_btree_small) -->
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat_btree_small_test <- predict(fit_btree_small, mh.test, n.trees = 100) -->
...
<!-- ```{r} -->
<!-- fit_btree_big <- gbm(f2, data = mh.train, -->
<!--       distribution = "gaussian", -->
<!--       n.trees = 100, -->
<!--       interaction.depth = 2, -->
<!--       shrinkage = 0.001) -->
...
<!-- ```{r} -->
<!-- yhat_btree_big <- predict(fit_btree_big, mh.train, n.trees = 100) -->
<!-- mse_btree_big <- mean((yhat_btree_big - y_train2) ^ 2) -->
<!-- print(mse_btree_big) -->
<!-- ``` -->
<!-- ```{r} -->
<!-- yhat_btree_big_test <- predict(fit_btree_big, mh.test, n.trees = 100) -->

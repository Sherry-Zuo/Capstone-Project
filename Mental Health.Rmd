...
dummydata<-dummy_cols(dummydata, select_columns = c("anonymity_protected", "awareness_mh_benefits","gender","medical_leave_easy", "mh_benefits", "mh_discuss", "mh_resources", "mh_serious_ph"))
```
```{r}
#after adding dummies, remove original categorical variables
dummydata <- dummydata[-c(2,11:17,23)]
```
```{r}
#We would try with state dummys or delete state when we do linear regression
dummydata<-dummy_cols(dummydata, select_columns = c("state"))
```
```{r}
#leave numeric variables and all dummies with state as dummies
...
midwest <- c("IL", "IN", "MI", "OH", "WI", "IA", "KS", "MN", "MO", "NE", "SD")

south <- c("DE", "FL", "GA", "MD", "NC", "SC", "VA","DC", "WV", "AL", "KY", "MS", "TN", "TX", "OK", "LA", "AR")

west <- c("AZ", "CO", "ID", "MT", "NV", "NM", "UT", "WY", "AK", "CA", "HI", "OR", "WA")

regions<- as.list(northeast, midwest, south, west)

maindata2$region <- ifelse(maindata2$state %in% c("CT", "ME", "MA", "NH", "RI", "VT", "NJ", "NY", "PA"), "NorthEast",
                           ifelse(maindata2$state %in% c("IL", "IN", "MI", "OH", "WI", "IA", "KS", "MN", "MO", "NE", "SD"),"MidWest",
                                  ifelse(maindata2$state %in% c("DE", "FL", "GA", "MD", "NC", "SC", "VA","DC", "WV", "AL", "KY", "MS", "TN", "TX", "OK", "LA", "AR"), "South",
                                         ifelse(maindata2$state %in% c("AZ", "CO", "ID", "MT", "NV", "NM", "UT", "WY","AK", "CA", "HI", "OR", "WA"), "West", NA))))


table(maindata2$region)
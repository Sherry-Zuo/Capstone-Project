...
```{r}
newdata<-maindata%>%
   mutate(mhtreatment=ifelse(mh_treatment=="Yes", 1, 0))%>%
   group_by(country)%>%
   summarize(treatment=mean(mhtreatment))
newdata
```
...
```{r}
summary(lm(treatment~h_score+suicide_rate, data=newdata))
```

...
set.seed(666)
# now split
mh.test <- maindata[test_index,]
mh.train <- maindata[-test_index,]
```
...
```{r}
y_train2 <- maindata$mh_treatment[-test_index]
y_test2 <- maindata$mh_treatment[test_index]
```

```{r}
f1 <- as.formula(mh_treatment ~ age+gender+country+tech_company+mh_discuss+company_size+remote+medical_leave_easy+family_history)
```
...
```{r}
# yhat.test.tree <- predict(fit.tree, mh_test)
# mse.test.tree <- mean((mh_test$mh_treatment - yhat.test.tree)^2)
# mse.test.tree
```
